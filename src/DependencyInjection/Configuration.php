                    ...
                    ->prototype('array')
                        ->children()
                            ...
                            ->scalarNode('metadata_cache')
                                ->info('PSR-6 cache service ID for metadata caching (optional)')
                                ->defaultNull()
                            ->end()
                            ->booleanNode('use_optimized_metadata')
                                ->info('Enable optimized metadata system for better performance')
                                ->defaultTrue()
                            ->end()
                            ->booleanNode('enable_value_objects')
                                ->info('Enable ValueObject support for automatic primitive conversion')
                                ->defaultTrue()
                            ->end()
                        ->end()
                    ->end()
